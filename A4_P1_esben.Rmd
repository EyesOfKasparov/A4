...
LoadData <- function(paths, threshold) {
  df <- tibble()
  for (path in paths) {
    data_path <- read_csv(path) %>% 
      Preprocess(path) %>% 
      mutate(Resp = RemoveOuts(Resp, threshold),
             ECG = RemoveOuts(ECG, threshold),
             HR = RemoveOuts(HR, threshold)) %>% 
      mutate(across(c(where(is.numeric), -time), ZScale))
    
    df <- df %>% rbind(data_path)
  }
  return(df)
}
