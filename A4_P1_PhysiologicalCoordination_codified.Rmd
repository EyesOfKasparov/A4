...
lmer1 <- lmerTest::lmer(delta_HR ~ 0 + (HR_self + HR_other):condition + (1|participant_id), 
                        data = master_data, 
...
master_join_data <- master_data %>% 
  ...
  distinct(join_key, .keep_all = T) %>% 
  select(join_key, HR_shuffled = HR_self)
...
real_master_data <- master_data_surrogate %>% 
  select(delta_HR, HR_self, HR_other, participant_id, condition) %>% 
  mutate(is_surrogate = F)

surrogate_master_data <- master_data_surrogate %>% 
  select(delta_HR, HR_self, HR_other=HR_shuffled, participant_id, condition) %>% 
  mutate(is_surrogate = T)
...
test_lmer_surrogate <- lmerTest::lmer(delta_HR ~ 1 + (HR_self + HR_other):condition:is_surrogate + (1|participant_id), joined_data)
