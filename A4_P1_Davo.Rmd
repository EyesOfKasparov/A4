...
master_data_ID <- master_data %>% 
  mutate(ID = paste(.groups, study_nr, Participant, sep = ''))
    
ggplot(master_data_ID) + 
  geom_path(aes(ID, HR, color = "Heart")) +
  labs(x = "time", y = "HR") + 
  theme_classic()

ggplot(master_data) +
  geom_path(aes(time, Resp, color = 'Resp')) +
  labs(x = 'time', y = 'Resp') +
  theme_light()
...
master_data_ID <- master_data_ID %>% 
  ...
  mutate(across(Resp:HR, lead, .names = "{.col}_lead")) %>% 
  mutate(across(Resp:HR, lag, .names = "{.col}_lag"))
...
# plotting
ggplot(master_data_ID, aes(x = delta_HR)) + 
  geom_histogram()

# Set the most interesting contrast e.g. by defining synchronous or conversation as the baseline

# Model change as a function of own and other previous state 
HR_model1 <- lm(delta_HR ~ 0 + (HR_self + HR_other) : condition, subset(master_data_ID, study_nr == "1" & Participant == 1))
...
master_data_ID_shuffled <- 
  ...
  mutate(
    ...
    across(Resp:HR_self, lead, .names = "{.col}_lead"),
    across(Resp:HR_self, lag, .names = "{.col}_lag"),
    Type = 'shuffled')

HR_model_shuffled <- lm(delta_HR ~ 0 + (HR_self + HR_other) : condition, subset(master_data_ID_shuffled, study_nr == "1" & Participant == 1))

summary(HR_model_shuffled)
